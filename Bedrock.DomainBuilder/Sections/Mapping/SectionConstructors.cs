...
namespace Bedrock.DomainBuilder.Sections.Mapping
{
        ...
        where T : struct, IConvertible
    {
        ...
        private void BuildConstructorMapping()
        {
            Add(Tab(2), string.Format("public {0}Map()", EntityNameSingularized));
            Add(Tab(2), "{");
        ...
        private void CreatePrimaryKeys()
        {
            ...
            if (Pass.EntityType.KeyMembers.Count() == 1)
            {
                ...
                Add(Tab(3), string.Format("this.HasKey(t => t.{0});", keyName));
            }
            else
            {
                ...
                else
                {
                    ...
                    Add(Tab(3), string.Format("this.HasKey(t => new {{ {0} }});", string.Join(", ", keys.Select(m => "t." + m.Name))));
                }
            }
        ...
        private void CreateProperties()
        {
            ...
            Pass.EntityType.Properties.Each(p =>
            {
                ...
                {
                    Add(Tab(3), string.Format("this.Property(t => t.{0}){2}{1};", propertyName, string.Join(newLineTab, propertyConfiguration), newLineTab), NewLine());
                }
            });
        }
        ...
        private void CreateTableAndColumnMappings()
        {
            ...
            {
                Add(Tab(3), string.Format("this.ToTable(\"{0}\");", tableName));
            }
            else
            {
                Add(Tab(3), string.Format("this.ToTable(\"{0}\", \"{1}\");", tableName, schemaName));
            }
            ...
            Pass.EntityType.Properties.Each(p =>
            {
                ...
                Add(Tab(3), string.Format("this.Property(t => t.{0}).HasColumnName(\"{1}\");", propertyName, Pass.PropertyToColumnMappings[p].Name));
            });
        ...
        private void CreateManyManyRelationships()
        {
            ...
            if (manyManyRelationships.Any())
            {
                manyManyRelationships.Each(r =>
                {
                    ...
                    Add(Tab(3), string.Format("this.HasMany(t => t.{0})", Pass.Code.Escape(r)));
