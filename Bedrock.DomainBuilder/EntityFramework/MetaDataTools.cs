...
namespace Bedrock.DomainBuilder.EntityFramework
{
    ...
    public class MetadataTools
    {
        ...
        public string UnderlyingClrTypeString(EdmType edmType)
        {
            switch (edmType.Name)
            {
                ...
                case "Edm.DateTime": return "DateTime";
                case "Edm.Binary": return "byte[]";
                case "Edm.Guid": return "Guid";

                default: throw new NotSupportedException("Not supported type " + edmType.Name);
            }
        }
        ...
        public bool IsNullable(TypeUsage typeUsage)
        {
            ...
            if (typeUsage != null && typeUsage.Facets.TryGetValue("Nullable", true, out nullableFacet))
                return (bool)nullableFacet.Value;

            return false;
        }

        public bool IsValueType(EdmType type)
        {
            var clrType = UnderlyingClrType(type);
        ...
        public int GetPropertyMaxLength(EdmProperty property)
        {
            int returnValue = 0;
            var maxLengthFacet = property.TypeUsage.Facets.FirstOrDefault(f => f.Name == "MaxLength");

            if (maxLengthFacet != null)
            {
                var maxLengthFacetType = maxLengthFacet.Value.GetType();

                if (maxLengthFacetType == typeof(int))
                    returnValue = (int)maxLengthFacet.Value;
            }
