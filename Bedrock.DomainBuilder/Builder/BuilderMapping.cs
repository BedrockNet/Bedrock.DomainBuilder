...
{
    public class BuilderMapping : BuilderBase, IBuilder
    {
        ...
        protected override async Task<int> BuildInternal(BuildSettings settings)
        {
            ...
            {
                var filePath = string.Concat(settings.MappingPath, file.Name, "Map", settings.FileExtension);

                await BuildFile<eBuildSectionMapping>(filePath, file, settings);
                await Pause.WaitWhilePausedAsync();
